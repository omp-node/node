...
eventEmitter_raw.on("playerRequestClass", async (badRet, player, classId) => {
  ...
  if (player_ === undefined)
    throw new Error(
      "Unable to cast player to Player for playerRequestClass. Value: " + player
    );
  const listeners = __internal_omp.eventEmitter.listeners("playerRequestClass");
  let result = true;
  for await (const listener of listeners) {
...
eventEmitter_raw.on(
  ...
  async (badRet, address, password, success) => {
    const listeners = __internal_omp.eventEmitter.listeners("rconLoginAttempt");
    let result = true;
    for await (const listener of listeners) {
...
eventEmitter_raw.on("playerFinishedDownloading", async (badRet, player, vw) => {
  const player_ = omp.players.get(player);
  if (player_ === undefined)
    throw new Error(
...
eventEmitter_raw.on(
  ...
  async (badRet, player, dialogId, response, listItem, inputText) => {
    ...
    let result = true;
    for await (const listener of listeners) {
      result = await listener(player_, dialogId, response, listItem, inputText);
      if (typeof result === "boolean" || typeof result === "number") {
        switch (badRet) {
          case 1:
            if (!result) {
...
eventEmitter_raw.on("playerEnterGangZone", async (badRet, player, zone) => {
  ...
  if (zone_ === undefined)
    throw new Error(
      "Unable to cast zone to GangZone for playerEnterGangZone. Value: " + zone
    );
  const listeners = __internal_omp.eventEmitter.listeners(
    "playerEnterGangZone"
  );
...
eventEmitter_raw.on("playerLeaveGangZone", async (badRet, player, zone) => {
  ...
  if (zone_ === undefined)
    throw new Error(
      "Unable to cast zone to GangZone for playerLeaveGangZone. Value: " + zone
    );
  const listeners = __internal_omp.eventEmitter.listeners(
    "playerLeaveGangZone"
  );
...
eventEmitter_raw.on("playerClickGangZone", async (badRet, player, zone) => {
  ...
  if (zone_ === undefined)
    throw new Error(
      "Unable to cast zone to GangZone for playerClickGangZone. Value: " + zone
    );
  const listeners = __internal_omp.eventEmitter.listeners(
    "playerClickGangZone"
  );
...
eventEmitter_raw.on(
  ...
  async (
    ...
    if (player_ === undefined)
      throw new Error(
        "Unable to cast player to Player for playerEditObject. Value: " + player
      );
    const object_ = omp.objects.get(object);
    if (object_ === undefined)
      throw new Error(
        "Unable to cast object to Object for playerEditObject. Value: " + object
      );
    const listeners = __internal_omp.eventEmitter.listeners("playerEditObject");
    let result = true;
    for await (const listener of listeners) {
...
eventEmitter_raw.on("playerPickUpPickup", async (badRet, player, pickup) => {
  ...
  if (player_ === undefined)
    throw new Error(
      "Unable to cast player to Player for playerPickUpPickup. Value: " + player
    );
  const pickup_ = omp.players.get(pickup);
  if (pickup_ === undefined)
    throw new Error(
      "Unable to cast pickup to Pickup for playerPickUpPickup. Value: " + pickup
    );
  const listeners = __internal_omp.eventEmitter.listeners("playerPickUpPickup");
  let result = true;
  for await (const listener of listeners) {
...
eventEmitter_raw.on("playerCancelTextDrawSelection", async (badRet, player) => {
  const player_ = omp.players.get(player);
  if (player_ === undefined)
    throw new Error(
...
eventEmitter_raw.on("playerClickTextDraw", async (badRet, player, textdraw) => {
  const player_ = omp.players.get(player);
  if (player_ === undefined)
    throw new Error(
...
eventEmitter_raw.on("playerCommandText", async (badRet, player, command) => {
  ...
  if (player_ === undefined)
    throw new Error(
      "Unable to cast player to Player for playerCommandText. Value: " + player
    );
  const listeners = __internal_omp.eventEmitter.listeners("playerCommandText");
  let result = true;
  for await (const listener of listeners) {
...
eventEmitter_raw.on("playerRequestSpawn", async (badRet, player) => {
  ...
  if (player_ === undefined)
    throw new Error(
      "Unable to cast player to Player for playerRequestSpawn. Value: " + player
    );
  const listeners = __internal_omp.eventEmitter.listeners("playerRequestSpawn");
  let result = true;
  for await (const listener of listeners) {
    result = await listener(player_);
    if (typeof result === "boolean" || typeof result === "number") {
...
eventEmitter_raw.on(
  ...
  async (badRet, player, weapon, x, y, z) => {
    ...
    if (player_ === undefined)
      throw new Error(
        "Unable to cast player to Player for playerShotMissed. Value: " + player
      );
    const listeners = __internal_omp.eventEmitter.listeners("playerShotMissed");
    let result = true;
    for await (const listener of listeners) {
...
eventEmitter_raw.on(
  ...
  async (badRet, player, target, weapon, x, y, z) => {
    ...
    if (player_ === undefined)
      throw new Error(
        "Unable to cast player to Player for playerShotPlayer. Value: " + player
      );
    const target_ = omp.players.get(target);
    if (target_ === undefined)
      throw new Error(
        "Unable to cast target to Player for playerShotPlayer. Value: " + target
      );
    const listeners = __internal_omp.eventEmitter.listeners("playerShotPlayer");
    let result = true;
    for await (const listener of listeners) {
...
eventEmitter_raw.on(
  ...
  async (badRet, player, target, weapon, x, y, z) => {
    ...
    if (player_ === undefined)
      throw new Error(
        "Unable to cast player to Player for playerShotObject. Value: " + player
      );
    const target_ = omp.players.get(target);
    if (target_ === undefined)
      throw new Error(
        "Unable to cast target to Player for playerShotObject. Value: " + target
      );
    const listeners = __internal_omp.eventEmitter.listeners("playerShotObject");
    let result = true;
    for await (const listener of listeners) {
      result = await listener(player_, target_, weapon, x, y, z);
      if (typeof result === "boolean" || typeof result === "number") {
...
eventEmitter_raw.on(
  ...
  async (badRet, player, from, amount, weapon, bodypart) => {
    ...
    if (player_ === undefined)
      throw new Error(
        "Unable to cast player to Player for playerTakeDamage. Value: " + player
      );
    ...
    const listeners = __internal_omp.eventEmitter.listeners("playerTakeDamage");
    let result = true;
    for await (const listener of listeners) {
...
eventEmitter_raw.on(
  ...
  async (badRet, player, to, amount, weapon, bodypart) => {
    ...
    if (player_ === undefined)
      throw new Error(
        "Unable to cast player to Player for playerGiveDamage. Value: " + player
      );
    ...
    const listeners = __internal_omp.eventEmitter.listeners("playerGiveDamage");
    let result = true;
    for await (const listener of listeners) {
...
eventEmitter_raw.on("vehicleStreamIn", async (badRet, vehicle, player) => {
  ...
  if (vehicle_ === undefined)
    throw new Error(
      "Unable to cast vehicle to Vehicle for vehicleStreamIn. Value: " + vehicle
    );
  const player_ = omp.players.get(player);
  if (player_ === undefined)
    throw new Error(
...
eventEmitter_raw.on("playerExitVehicle", async (badRet, player, vehicle) => {
  ...
  if (player_ === undefined)
    throw new Error(
      "Unable to cast player to Player for playerExitVehicle. Value: " + player
    );
  const vehicle_ = omp.vehicles.get(vehicle);
  if (vehicle_ === undefined)
    throw new Error(
  ...
  const listeners = __internal_omp.eventEmitter.listeners("playerExitVehicle");
  let result = true;
  for await (const listener of listeners) {
...
eventEmitter_raw.on(
  ...
  async (badRet, player, vehicle, paintJob) => {
    ...
    if (player_ === undefined)
      throw new Error(
        "Unable to cast player to Player for vehiclePaintJob. Value: " + player
      );
    const vehicle_ = omp.vehicles.get(vehicle);
    if (vehicle_ === undefined)
      throw new Error(
    ...
    const listeners = __internal_omp.eventEmitter.listeners("vehiclePaintJob");
    let result = true;
    for await (const listener of listeners) {
...
eventEmitter_raw.on(
  ...
  async (badRet, player, enterexit, interiorId) => {
    ...
    if (player_ === undefined)
      throw new Error(
        "Unable to cast player to Player for enterExitModShop. Value: " + player
      );
    const listeners = __internal_omp.eventEmitter.listeners("enterExitModShop");
    let result = true;
    for await (const listener of listeners) {
