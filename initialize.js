...
async function onResourceStart(badRet, error) {
  if (!error) {
  ...
  const eventEmitter = __internal_omp.eventEmitter;
...
eventEmitter_raw.removeListener("resourceStart", onResourceStart);
eventEmitter_raw.on("resourceStart", onResourceStart);
